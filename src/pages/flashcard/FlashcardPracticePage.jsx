...
import useFlashcardPractice from '../../hooks/flashcard/useFlashcardPractice';
...
function FlashcardPracticePage() {
  ...
  const [correntAnswersCount, setCorrectAnswersCount] = useState(0);
  const [imageUrl, setImageUrl] = useState('');
  

  const { list } = useFlashcardPractice({ listId });
  ...
  async function handleFinish() {
    const response = await recordPractice("FLASHCARD", listId, correntAnswersCount, list.cards.length);
    if(!response) 
      return;
  ...
  const imageSizeInPixels = 50 + Math.min(streak * 7, 35); // Base 50, grows by 7 per streak, max +35 (so max 85)

  const streakNumberFontSize = `${1 + Math.min(streak * 0.05, 0.5)}rem`;  // Base 1rem, grows by 0.05rem per streak, max +0.5rem

  return (
     <div className="min-h-screen bg-slate-200 dark:bg-zinc-700 text-white p-6">
        {/* Header */}
        <div className="flex justify-between items-center mb-6">
            ...
            {/* Progress Bar */}
            <div className="w-full max-w-md h-2 bg-gray-700 dark:bg-gray-200 rounded-full overflow-hidden">
                <div
              ...
              <div className="flex items-center min-w-100px"> {/* Container for streak and title, min-w to prevent squish */}
                ...
                <div
                  ...
                  className={`flex items-center text-orange-400 font-bold sm:mr-4 ${playStreakAnimation ? 'streak-animate-pop' : ''}`}
                >
                ...
                {imageUrl ? (  
                  ...
                  className="rounded-full sm:mr-1 streak-icon-fire"
                  style={{ transition: 'font-size 0.3s ease-out' , width: `${imageSizeInPixels}px`, height: `${imageSizeInPixels}px` }}
                  />
                  <span className="text-gray-900 dark:text-white" style={{ fontSize: streakNumberFontSize, transition: 'font-size 0.3s ease-out'}}>
                    {streak}
                  </span>
                  </>
                  ) : 
        ...
        <div className="h-120 bg-slate-100 dark:bg-zinc-800 text-white flex flex-col items-center justify-center p-6 rounded-lg">
            {currentCard && (
            <div className="relative w-full max-w-md perspective-normal perspective-origin-top">
                <div
                    ...
                    {/* Front */}
                    <div className="border-x-1 dark:border-x-0 border-t-1 dark:border-t-0 bg-gray-300/70 dark:bg-gray-950 border-b-4 border-indigo-500 dark:border-indigo-700 absolute w-full h-full backface-hidden text-gray-900 dark:text-gray-100 text-2xl font-semibold flex items-center justify-center rounded-xl shadow-xl">
                        {list.defaultLanguage === 'ENGLISH' ? currentCard.englishWord : currentCard.malayWord}
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-purple-600 to-indigo-700 absolute w-full h-full backface-hidden text-white text-2xl font-semibold flex items-center justify-center rounded-xl shadow-xl transform rotate-y-180">
                        {list.defaultLanguage === 'ENGLISH' ? currentCard.malayWord : currentCard.englishWord}
                    </div>
                </div>
                {!showAnswer && (
                <button
                ...
                {showAnswer && !feedback && (
                  <div className="mt-4 text-center">
                    <div className="text-gray-900 dark:text-white font-medium mb-2">Did you get it right?</div>
                ...
                <div className="mt-4 text-center">
                {feedback === 'yes' ? (
                    <div className="flex items-center justify-center gap-2 text-green-600 dark:text-green-400 text-lg font-medium">
                        <FaSmile size={24} /> Great job!
                    </div>
                    ) : (
                    <div className="flex items-center justify-center gap-2 text-red-500 dark:text-red-400 text-lg font-medium">
                ...
                <button
                  ...
                  Finish Practice 
                  {/* <span className='text-base'>(+{streak} EXP)</span> */}
                </button>
              )}
                </div>
                )}
